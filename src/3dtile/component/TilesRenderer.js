...
export class TilesRenderer extends TilesRendererBase {
	...
	setCamera( camera ) {
		...
		if ( ! cameraMap.has( camera ) ) {
			...
			cameraMap.set( camera, new THREE.Vector2() );
			cameras.push( camera );
			return true;
	...
	setResolution( camera, xOrVec, y ) {
		...
		if ( xOrVec instanceof THREE.Vector2 ) {
	...
	preprocessNode( tile, parentTile, tileSetDir ) {
		...
		const transformInverse = new THREE.Matrix4().copy( transform ).invert();
		...
		if ( 'box' in tile.boundingVolume ) {
			...
			box = new THREE.Box3();
			boxTransform = new THREE.Matrix4();
			boxTransformInverse = new THREE.Matrix4();

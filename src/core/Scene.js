...
import {EarthAutoRotate} from "./EarthAutoRotate";
import {isGlobal} from "../util/Utils";
...
function cesium2threeBufferGeometry(geometry,result) {
    ...
    for (const name in attributes) {
        if (attributes.hasOwnProperty(name) && attributes[name]) {
            ...
            const attribute3js = new THREE.BufferAttribute(array, attribute.componentsPerAttribute, attribute.normalize);
...
function cesium2threeGeometry(geometry) {
    ...
    for (let i = 0; i < positions.length; i += 3) {
        positionIdx = i * 3;
        result.vertices.push(new THREE.Vector3(positions[positionIdx], positions[positionIdx + 2], positions[positionIdx + 1]));
    }

    for (let i = 0; i < geometry.indices.length; i += 3) {
        const idx1 = geometry.indices[i];
        const idx2 = geometry.indices[i + 1];
        const idx3 = geometry.indices[i + 2];
        result.faces.push(new THREE.Face3(idx1, idx2, idx3));
    }
    return result;
}

function initCesium(domID, option = {}) {
    ...
    function  DateTimeFormatter(datetime, viewModel, ignoredate) {
        ...
        return formatDate(new Date(
            ...
            gregorianDT.day,
            gregorianDT.hour,
            gregorianDT.minute-1,
            gregorianDT.millisecond
        ),fms);
    }
...
class Scene extends THREE.Scene {
    constructor(options = {}) {
        ...
        if (!options.camera || !(options.camera instanceof THREE.PerspectiveCamera)) {
            throw new Error('THREE.ThreeCesiumScene (not found cesiumDom) OR ( not THREE.PerspectiveCamera ).')
        }
        if (!options.renderer) {
        ...
        //cesium
        const threeCesiumScene=this;
        const keys=['imageryProvider','terrainProvider'];
        function findCesiumProvider(options) {

            const object={};

            for (const key in options) {
                if (keys.includes(key)&&options[key]&&options[key].type){
                    object[key]=threeCesiumScene.createCesiumProvider(options[key]);
                    continue;
                }
        ...
        //cesium视图
        this.cesiumViewer = initCesium((typeof options.cesiumDom === 'string') ? options.cesiumDom : options.cesiumDom.id, findCesiumProvider(options.cesiumOption));

        const doms=(typeof options.cesiumDom === 'string') ? document.getElementById(options.cesiumDom) : options.cesiumDom;
        ...
        if (!!options.threeHabit){
            ...
            // 倾斜视图 鼠标左键旋转
            this.cesiumViewer.scene.screenSpaceCameraController.tiltEventTypes = [ Cesium.CameraEventType.LEFT_DRAG ];

            // 缩放设置 重新设置缩放成员
            this.cesiumViewer.scene.screenSpaceCameraController.zoomEventTypes = [Cesium.CameraEventType.MIDDLE_DRAG, Cesium.CameraEventType.WHEEL, Cesium.CameraEventType.PINCH];

            // 平移 添加鼠标右键  鼠标右键平移
            this.cesiumViewer.scene.screenSpaceCameraController.rotateEventTypes = [Cesium.CameraEventType.RIGHT_DRAG];
        }

        //自动旋转
        this.autoRotateControl=new EarthAutoRotate(this.cesiumViewer);
        if (options.autoRotate){
            this.autoRotateControl.enabled=true;
            if (options.rotateSpeed) this.autoRotateControl.speed=Number(options.rotateSpeed);
        }
        ...
        const ellipsoid = new Cesium.EllipsoidGeometry({
            ...
            radii: new Cesium.Cartesian3(this.ellipsoid.maximumRadius-1, this.ellipsoid.maximumRadius-1, this.ellipsoid.minimumRadius-1),
            slicePartitions: 1024,//8132,
            stackPartitions: 1024
        });
        const geometry = cesium2threeBufferGeometry(Cesium.EllipsoidGeometry.createGeometry(ellipsoid));
        ...
        //补偿地板覆盖
        const geometry3 = new THREE.SphereGeometry(this.ellipsoid.minimumRadius, 512, 256, 0, Math.PI * 2, 3.1, Math.PI -3.1);
        ...
        this.sun=new THREE.DirectionalLight(this.sunLightColor,this.sunLightIntensity);
        ...
        if (options.enableLighting !== undefined && options.enableLighting !== null){
        ...
        if (options.cesiumLayers&&options.cesiumLayers.length>0){
            for (let i = 0; i < options.cesiumLayers.length; i++) {
                this.addImageryProvider(options.cesiumLayers[i].type,options.cesiumLayers[i].option);
            }
        }
    ...
    updateSunMatrix(){
        ...
        this.sunUp.lookAt(c3.x-this.cameraOffset.x,c3.y-this.cameraOffset.y,c3.z-this.cameraOffset.z);
    ...
    updateCameraMatrix() {
        //同步相机
        this.camera.fov = Cesium.Math.toDegrees(this.cesiumViewer.camera.frustum.fovy) // ThreeJS FOV is vertical
    ...
    updateGroupMatrixWorld() {
        // 得到面向模型的前向方向
        const center = this.cartesian3ToVector(Cesium.Cartesian3.fromDegrees(this.lngLat[0], this.lngLat[1], 0));
        // 使用从左下到左上的方向作为上向量
        const topLeft = this.cartesian3ToVector(Cesium.Cartesian3.fromDegrees(this.lngLat[0], this.lngLat[1], 2));
    ...
    toJSON(meta) {
        ...
        if (this.childrenGroup.children.length > 0) {
            ...
            for (let i = 0; i < this.childrenGroup.children.length; i++) {
                object.children.push(this.childrenGroup.children[i].toJSON(meta).object);
            }
        }
    ...
    addImageryProvider(type,option){
        this.cesiumViewer.imageryLayers.addImageryProvider(this.createCesiumProvider({
            type,
            option
        }));
    }
